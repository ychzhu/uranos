...
#ifdef PREFIX
   configFilePath = TOSTRING(PREFIX);
#elif
#endif
*/
...
void MainWindow::exportSettings(string str)
{
    ...
    *stream_out << endl;
    *stream_out << "// generated by URANOS "<<versionString<<" on " << QSysInfo::prettyProductName().toStdString();
                   // " "<< QSysInfo::productVersion().toStdString()<< " "<< QSysInfo::productType().toStdString()
    *stream_out<< " ("<< QSysInfo::kernelType().toStdString() << " "<< QSysInfo::kernelVersion().toStdString() << QSysInfo::currentCpuArchitecture().toStdString()<<")" <<endl;
    ...
    if (QSysInfo::kernelType() == "linux") //seems to not work properly (output full cpuinfo, not piped)
    {
...
void MainWindow::on_pushButton_about_clicked()
{
    ...
    messageString+= QString::fromStdString(versionString);
    messageString+=        "<br> ";
